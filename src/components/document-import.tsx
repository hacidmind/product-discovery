...
const TYPE_CONFIG: Record<string, { label: string; icon: string; variant: "accent" | "success" | "warning" | "danger" | "default" }> = {
  interview: { label: "Interview", icon: "\u25C9", variant: "accent" },
  experiment: { label: "Experiment", icon: "\u2726", variant: "success" },
            ...
            >
              ...
              {file ? (
                <div className="space-y-3">
                  ...
                  <button type="button" className="text-xs underline" onClick={(event) => { event.stopPropagation(); handleReset(); }}>
                    Remove
                  </button>
                </div>
              ) : (
        ...
        <div className="space-y-4">
          <Card>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              {Object.entries(TYPE_CONFIG).map(([key, config]) => {
                const count = (result.summary as unknown as Record<string, number>)[(key === "opportunity" ? "totalOpportunities" : `total${key.charAt(0).toUpperCase() + key.slice(1)}s`)] || 0;
                return (
                  ...
                  <Card key={item.data.id || index} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[var(--text-tertiary)] mt-0.5">
                        {item.type === "interview" && "Added to Interviews"}
                        {item.type === "experiment" && "Added to Experiments"}
