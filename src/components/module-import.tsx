  ...
  return <>
    {enabled && pathname !== "/import" && <section className="mx-auto mb-6 max-w-6xl rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" className="rounded-lg border border-[var(--border)] px-3 py-2 text-sm font-medium hover:bg-[var(--bg-hover)]" aria-expanded={open} aria-controls="module-document-import" onClick={() => setOpen(!open)}>{open ? "Hide file insights" : "Generate from file"}</button>
      </div>
      <div id="module-document-import" hidden={!open} className="mt-5"><DocumentImport onImported={() => setRevision(value => value + 1)} /></div>
    </section>}
