...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return <WorkspaceContext.Provider value={{ openWorkspace: selectProduct, createWorkspace: () => { setDraftProduct(""); setChooseProduct(true); } }}><ToastProvider><MotionLibraries /><a href="#main-content" className="skip-link">Skip to content</a><div className="flex min-h-screen overflow-hidden bg-[var(--bg)]">
    <aside id="workspace-navigation" inert={!sidebarOpen} className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-[var(--border)] bg-[var(--bg-secondary)] transition-transform duration-300 md:static ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:hidden"}`} aria-label="Main navigation">
      <div className="flex h-[72px] items-center gap-3 border-b border-[var(--border)] px-5"><div className="grid size-9 place-items-center rounded-xl bg-[var(--accent)] text-[var(--bg)]"><Sparkles size={18} /></div><div><p className="font-display text-sm font-bold tracking-tight">Product Discovery</p><button onClick={() => { router.push("/library"); }} className="block max-w-40 truncate text-left text-[10px] uppercase tracking-[.15em] text-[var(--accent)] hover:underline">{productName || "Choose workspace"}</button></div></div>
    ...
    <div className="flex min-w-0 flex-1 flex-col"><header className="flex h-[72px] items-center gap-3 border-b border-[var(--border)] bg-[var(--bg)] px-4 md:px-7"><button onClick={() => setSidebarOpen(prev => !prev)} className="grid size-9 place-items-center rounded-lg text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]" aria-label="Toggle navigation" aria-expanded={sidebarOpen} aria-controls="workspace-navigation">{sidebarOpen ? <PanelLeftClose size={18} /> : <PanelLeftOpen size={18} />}</button><div className="flex min-w-0 items-center gap-2.5"><div className="min-w-0"><p className="font-display text-base font-semibold">{page}</p><p className="max-w-[45vw] truncate text-[11px] text-[var(--text-tertiary)]">{product ? `Workspace: ${productName}` : "Choose a workspace to begin"}</p></div>{product && <button onClick={() => { router.push("/library"); }} title="View all your research workspaces" className="rounded-lg border border-[var(--border)] px-2.5 py-1.5 text-xs font-medium text-[var(--text-secondary)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]">Switch</button>}</div><Link href="/search" className="ml-auto hidden items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] px-3 py-2 text-xs text-[var(--text-tertiary)] sm:flex"><Search size={14} />Search <kbd className="ml-5 font-mono">Ctrl K</kbd></Link></header><main id="main-content" tabIndex={-1} className="flex-1 overflow-y-auto p-4 md:p-7">{workspaceError && !chooseProduct && <div role="alert" className="mb-4 rounded-xl border border-[var(--danger)] p-4 text-sm">{workspaceError} <button className="ml-2 underline" onClick={() => window.location.reload()}>Reload</button></div>}{ready ? (product || pathname === "/library" ? <WorkspaceMotion key={product + pathname}>{children}</WorkspaceMotion> : <div className="mx-auto max-w-lg py-16 text-center"><h1 className="font-display text-2xl font-semibold">Your next discovery starts here</h1><p className="my-4 text-sm text-[var(--text-secondary)]">Choose a workspace to keep your research, evidence, and experiments together.</p><button className="rounded-lg bg-[var(--accent)] px-4 py-3 text-[var(--bg)]" onClick={() => router.push("/library")}>View your research</button></div>) : <div className="flex h-full min-h-[40vh] items-center justify-center"><Spinner size={24} /></div>}</main></div>
    <Modal open={mounted && ready && chooseProduct} onClose={() => { if (!saving) setChooseProduct(false); }} title={products.length ? "Choose your workspace" : "Create your first workspace"}>
      <p className="text-sm leading-6 text-[var(--text-secondary)]">A workspace keeps one product or initiative&apos;s research together. Give it a name your colleagues will recognize. Workspaces are private to your account.</p>
      {products.length > 0 && <div className="mt-5 space-y-2">{products.map(item => <button disabled={saving} key={item.id} onClick={() => selectProduct(item)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-[var(--border)] p-4 text-left hover:border-[var(--accent)] hover:bg-[var(--bg-hover)]"><span className="min-w-0"><span className="block truncate text-sm font-medium">{item.name}</span><span className="mt-1 block text-xs text-[var(--text-secondary)]">{item.researchCount || 0} research reports</span></span><span className="text-xs text-[var(--accent)]">Open &rarr;</span></button>)}</div>}
      <form className="mt-6 border-t border-[var(--border)] pt-5" onSubmit={event => { event.preventDefault(); void saveProduct(); }}>
        <label htmlFor="workspace-name" className="block text-sm font-medium">New workspace name</label>
        <input id="workspace-name" required maxLength={100} disabled={saving} value={draftProduct} onChange={event => setDraftProduct(event.target.value)} placeholder="e.g. Customer onboarding" className="mt-2 w-full rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 py-3 text-sm" />
        {workspaceError && <p role="alert" className="mt-3 text-sm text-[var(--danger)]">{workspaceError}</p>}
        <button type="submit" disabled={saving || !draftProduct.trim()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-[var(--bg)] disabled:opacity-50">{saving && <Spinner />}{saving ? "Creating workspace..." : "Create workspace"}</button>
      </form>
    </Modal>
