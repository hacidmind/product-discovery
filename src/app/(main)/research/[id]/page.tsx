...
import { checkedFetch as fetch, downloadFile } from "@/lib/api-client";
...
export default function ResearchWorkspacePage() {
  ...
  useEffect(() => { fetch(`/api/research/${id}`).then(async response => { if (!response.ok) throw new Error("Not found"); const data = await response.json() as ResearchResult; setResearch(data); setNotes(data.notes ?? ""); }).catch(error => { setError(error instanceof Error ? error.message : "Could not load the report."); setResearch(null); }).finally(() => setLoading(false)); }, [id]);
  const saveNotes = async () => { if (!research) return; setSaving(true); setSaved(false); setError(""); try { const response = await fetch(`/api/research/${research.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ notes }) }); if (!response.ok) throw new Error("Save failed"); const updated = await response.json() as ResearchResult; setResearch(updated); setNotes(updated.notes ?? ""); setSaved(true); } catch (error) { setError(error instanceof Error ? error.message : "Could not save notes. Please try again."); } finally { setSaving(false); } };
  const download = async () => {
    if (!research || downloading) return;
    setDownloading(true); setError("");
    try { await downloadFile(`/api/research/${research.id}/download`, research.savedFile || "research-report.md"); }
    catch (error) { setError(error instanceof Error ? error.message : "Could not download the report."); }
    finally { setDownloading(false); }
  };
  if (loading) return <div className="flex justify-center py-24"><Spinner size={25}/></div>;
  if (!research) return <div className="mx-auto max-w-xl py-20 text-center"><Search size={30} className="mx-auto mb-4 text-[var(--accent)]"/><h1 className="font-display text-xl font-semibold">Research not found</h1><p className="mt-2 text-sm text-[var(--text-secondary)]">{error || "This research record may have been deleted."}</p><Link className="mt-5 inline-block text-sm text-[var(--accent)]" href="/research">Back to research</Link></div>;
  const category = research.category.replace(/_/g, " ");
  return <div className="mx-auto max-w-5xl animate-fadein"><ModuleError message={error} />
    <Link href="/library" className="mb-6 inline-flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text)]">← All research</Link>
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><div className="mb-3 flex items-center gap-2 text-xs font-medium uppercase tracking-[.15em] text-[var(--accent)]"><BrainCircuit size={15}/>Research workspace</div><h1 className="font-display text-3xl font-semibold tracking-tight">{research.product || "Untitled research"}</h1><p className="mt-2 max-w-3xl text-sm text-[var(--text-secondary)]">{research.query}</p><div className="mt-3 flex flex-wrap gap-2"><Badge variant="accent">{category}</Badge><Badge variant="default">{new Date(research.createdAt).toLocaleDateString()}</Badge><Badge variant="default">{research.sources.length} sources</Badge></div></div><Button variant="secondary" onClick={download} disabled={downloading}><FileDown size={15}/>{downloading ? "Preparing report..." : "Download report"}</Button></div>
