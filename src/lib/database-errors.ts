...
export function databaseErrorMessage(error: unknown): string {
  const detail = error as { code?: string; name?: string; syscall?: string; cause?: { code?: string; syscall?: string } } | null;
  ...
  if (code === "ETIMEOUT" || code === "ETIMEDOUT" || detail?.name === "MongoServerSelectionError") {
    return "MongoDB could not be reached within 15 seconds. Check Atlas Network Access, your connection string, and whether your network allows port 27017.";
  }
...
export function databaseFailureResponse(error: unknown) {
  ...
  if (!/^(Mongo|Mongoose)/.test(detail?.name ?? "") && !/querySrv|MONGODB_URI/.test(message) && !["ESERVFAIL", "ENOTFOUND", "EAI_AGAIN", "ETIMEOUT", "ETIMEDOUT"].includes(detail?.code ?? detail?.cause?.code ?? "")) return null;
